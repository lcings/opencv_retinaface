...
int AnchorGenerator::FilterAnchor(cv::Mat *cls, cv::Mat *reg, \
	cv::Mat *pts, std::vector<Anchor>& result, float ratio_w, float ratio_h, float confidence_threshold) {


	ratiow = ratio_w;
	ratioh = ratio_h;
	assert(cls->size[1] == anchor_num * 2);   //anchor_num=2
